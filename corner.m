...
function [points] = corner (I, varargin)
  ...
  if (strcmp(method, "Harris"))
    ...
    for i = 1:size(I)(1)
      for j = 1:size(I)(2)
        Q(i, j) = eigs ([A(i, j) C(i, j); C(i, j) B(i, j)], 1, "sm");
      endfor
    endfor
  endif
...
%! imshow (I);
%! hold;
%! p = corner (I, "MinimumEigenvalue", "FilterCoefficients", [1 2 1]);
%! plot (p(:,2), p(:,1),"marker", "o", "color", "r", "linestyle", "none");
